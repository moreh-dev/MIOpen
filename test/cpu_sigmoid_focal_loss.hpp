                                               ...
                                               float gamma = 2)
{
    ...
    for(size_t id = 0; id < inputSize; ++id)
    {
        ...
        float dpdi      = std::exp(-i) / pow(1 + std::exp(-i), 2);
                                     ...
                                     float divisor = 1)
{
    ...
    auto target_tv  = miopen::get_inner_expanded_tv<5>(target.desc);
    auto doutput_tv = miopen::get_inner_expanded_tv<5>(doutput.desc);
    auto dinput_tv  = miopen::get_inner_expanded_tv<5>(dinputHost.desc);
    ...
    for(size_t id = 0; id < inputSize; ++id)
    {
        ...
        dinputHost[dinput_tv.get_tensor_view_idx(idx)] = static_cast<TIO>(grad);
    }
}
