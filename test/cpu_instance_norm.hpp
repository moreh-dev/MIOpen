        ...
        par_ford(LOCAL_SIZE)([&](uint64_t lid) {
            ford(outer_size)([&](uint64_t i) {
                for(uint64_t j = lid; j < inner_size; j += LOCAL_SIZE)
                {
                    ...
                    outputHost[yidx] = static_cast<T>((static_cast<float>(input[xidx]) - pmean) *
                                ...
                                const tensor<T> meanVar)
{
    ...
    par_ford(gws)([&](uint64_t gid) {
        ...
        ford(outer_size)([&](uint64_t i) {
            ...
            par_ford(LOCAL_SIZE)([&](uint64_t lid) {
                pmean[lid] = static_cast<float>(
                    meanVar[mean_var_tv.stride[1] * (gid * 2) + mean_var_tv.stride[0] * i]);
                pvar[lid] = static_cast<float>(
                    meanVar[mean_var_tv.stride[1] * (gid * 2 + 1) + mean_var_tv.stride[0] * i]);
            });
